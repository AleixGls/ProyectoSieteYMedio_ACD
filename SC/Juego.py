    ...
    while True:
        ...
        if option == "1":
            ...
            while True:
                try:
                    ...
                    if new_bet > 0 and new_bet <= player["points"]:
                        ...
                        print(
                            f"Invalid bet. Must be a positive number and not greater than your current points ({player['points']}).".center(
                                127))
                        time.sleep(1.2)
                except ValueError:
            ...
            while True:
                ...
                time.sleep(1.2)
                addToRoundHistory(
                    str(f"{player["name"]} has received: {Dt.context_game['cards_deck'][card]['literal']}."))
                addToRoundHistory(str(f"Current points of {player["name"]} in this round: {player['roundPoints']}"))
                if player["roundPoints"] > 7.5:
...
def distributionPointAndNewBankCandidates():
    ...
    if candidates:
        ...
        if Dt.context_game["game"]:
            ...
            Dt.context_game["players"][new_bank_id]["bank"] = True
            print(
                f"{Dt.context_game['players'][new_bank_id]['name']} is the new bank because the previous one was eliminated.")
            addToRoundHistory(
                str(f"{Dt.context_game['players'][new_bank_id]['name']} is the new bank because the previous one was eliminated."))
            time.sleep(1.2)
...
def bankOrderNewCard(id, mazo):
    ...
    if all(player["roundPoints"] >= Dt.context_game["players"][other_id]["roundPoints"] for other_id in
           Dt.context_game["game"] if other_id != id):
        return False
...
def printStats(idPlayer="", titulo=""):
    ...
    while True:
        ...
        print("Name".center(16) + "Human".center(16) + "Priority".center(15) + "Type".center(16) + "Bank".center(
            16) + "Bet".center(16) + "Points".center(16) + "Round Points".center(16))
        print("-" * 127)
        ...
        for player_id in players_to_show:
            player = Dt.context_game["players"][player_id]
            print(
                player["name"].center(16) + str(player["human"]).center(16) + str(player["priority"]).center(15) + str(
                    player["type"]).center(16) + str(player["bank"]).center(16) + str(player["bet"]).center(16) + str(
                    player["points"]).center(16) + str(player["roundPoints"]).center(16))
        print()
        input("Press Enter to return...".center(127))
        break
...
def checkMinimun2PlayerWithPoints():
    players_with_points = [player_id for player_id in Dt.context_game["game"] if
                           Dt.context_game["players"][player_id]["points"] > 0]
            ...
            while True:
                try:
                    ...
                    if new_bet > 0 and new_bet <= player["points"]:
                        ...
                        print(
                            f"Invalid bet. Must be a positive number and not greater than your current points ({player['points']}).")
                except ValueError:
                    print("Invalid entry. Enter a number.")
        else:
...
def setInitialPriorities(mazo):
    ...
    for player_id in Dt.context_game["game"]:
        ...
        Dt.context_game["players"][player_id]["priority"] = Dt.context_game["cards_deck"][card]["priority"]
        Dt.context_game["players"][player_id]["realValue"] = Dt.context_game["cards_deck"][card][
            "realValue"]  # Guardar el valor real de la carta
...
def dealInitialCardToPlayers(mazo):
    ...
    for player_id in Dt.context_game["game"]:
        ...
        Dt.context_game["players"][player_id]["roundPoints"] += Dt.context_game["cards_deck"][card]["value"]
        print(
            f"{Dt.context_game['players'][player_id]['name']} has recived: {Dt.context_game['cards_deck'][card]['literal']}")
        addToRoundHistory(
            str(f"{Dt.context_game['players'][player_id]['name']} has recived: {Dt.context_game['cards_deck'][card]['literal']}"))
        time.sleep(1.2)
